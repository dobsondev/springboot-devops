...
There is also an automatic continuous delivery mechanism setup. If GitHub detects changes to either the `./springboot` or `./react-app` directories when merged to main then either the `./github/workflows/cd-push-springboot-image.yml` or `./github/workflows/cd-push-react-image.yml` will automatically trigger. Both of these workflows do the same thing as the manual "Push Docker Images to DockerHub" workflow but automatically when changes happen.
